...
<div class="container">
    ...
    <div class="row col-md-6 col-md-offset-3" id="body">
        <!--<h1 id="temp"></h1>-->
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    $(document).ready(function () {
        ...
        documentArea = documentHeight * documentWidth;
    });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.lists.length; i++) {
                        ...
                        result[index4Result] = '<h3 class="chapter" id="chapter_' + (i + pageStart - num - start) + '" data-rank="' + data.lists[i].chapterCount + '">'
                        ...
                        for (var j = 0; j < data.lists[i].sections.length; j++) {
                            result[index4Result] = '<p>' + data.lists[i].sections[j] + '</p>';
                            index4Result += 1;
                        }
                        result[index4Result] = '</br>';
                        index4Result += 1;
                    ...
                    setTimeout(function () {
                        ...
                        $('#body').append('<div class="lists" id="lists_' + index4lists + '"></div>');
                        var objlists = $('#lists_' + index4lists);
                        index4lists += 1;

                        for (var i = 0; i < result.length; i++) {
                            var listArea = objlists.height() * objlists.width() * 2;

                            if (listArea > documentArea) {
                                $('#body').append('<div class="lists" id="lists_' + index4lists + '"></div>');
                                objlists = $('#lists_' + index4lists);
                                index4lists += 1;
                            }

                            objlists.append(result[i]);
                        }
